...
def check_UdatasetFull():
    cnt = len([name for name in os.listdir("/home/pi/Documents/OpenCV-Face-Recognition-master/FacialRecognition/datasetU") if os.path.isfile(name)])
    cnt = cnt/30
    if ((cnt/30) <10 ):
        return False
    else:
...
cascadePath = "/home/pi/Documents/OpenCV-Face-Recognition-master/FacialRecognition/haarcascade_frontalface_default.xml"
...
while (True and (time.time()-NoBody)<10):
...
#            if (confi  < 70):
            ...
            if flu!= 2:
                ...
                if (check_UdatasetFull()):
                    tkinter.messagebox.showinfo("Error","The unknown's dataset is full")
                else:
